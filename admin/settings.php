...
if ( isset($_REQUEST['submit']) || isset($_REQUEST['libsyn_settings_submit']) ) { //has showSelect on form.
	...
	if ( $api instanceof \Libsyn\Api ) { //Brand new setup or changes?
		if ( !empty($_REQUEST['submit']) && (($_REQUEST['submit']==='Save Player Settings' || $_REQUEST['submit']==='Save+Player+Settings') || ($_REQUEST['submit']==='Save Settings' || $_REQUEST['submit']==='Save+Settings')) ) { //has Player Settings Update
			...
			if ( !isset($_REQUEST['player_use_thumbnail']) ) {
				$playerSettings['player_use_thumbnail'] = '';
			} else {
				$playerSettings['player_use_thumbnail'] = $_REQUEST['player_use_thumbnail'];
			}
			...
			if ( !isset($_REQUEST['player_use_download_link']) ) {
				$playerSettings['player_use_download_link'] = '';
			} else {
				$playerSettings['player_use_download_link'] = $_REQUEST['player_use_download_link'];
			}
			...
			/* DISABLE MYLIBSYN WIDGET */
			/*
			if ( !isset($_REQUEST['settings_enable_mylibsyn_widget']) ) {
				$additionalSettings['settings_enable_mylibsyn_widget'] = '';
			} else {
				$additionalSettings['settings_enable_mylibsyn_widget'] = $_REQUEST['settings_enable_mylibsyn_widget'];
			}
			*/
...
} elseif ( empty($api) && empty($clientId) && ( is_null($api) || $api === false) ) {
	$render = true;
}
...
<?php wp_enqueue_script( 'jquery-easing', plugins_url(LIBSYN_DIR . '/lib/js/jquery.easing.min.js')); ?>
	...
	<div class="wrap">
	  ...
	  <h2><?php _e("Publisher Hub - Settings", $libsyn_text_dom); ?><span style="float:right;margin-right:6.7%;"><a href="http://www.libsyn.com/"><img src="<?php _e(plugins_url( LIBSYN_DIR . '/lib/images/libsyn_dark-small.png'), $libsyn_text_dom); ?>" title="Libsyn Podcasting" height="28px"></a></span></h2>
	  <form name="<?php echo LIBSYN_NS . "form" ?>" id="<?php echo LIBSYN_NS . "form" ?>" method="post" action="">
		 <div id="poststuff">
		  <div id="post-body">
			...
			<!-- BOS Existing API -->
			  <div class="stuffbox libsyn-meta-box" style="width:93.5%">
				...
				<div class="inside" style="margin: 15px;">
				  <p><em><?php echo __("For assistance in configuring the Libsyn Publisher Hub, please review to our guide ", $libsyn_text_dom) . "<a style=\"font-size: inherit;\" href=\"https://support.libsyn.com/knowledgebase/libsyn-publisher-hub/\" target=\"_blank\" title=\"" . __("Libsyn Publisher Hub - Knowledge Base", $libsyn_text_dom) . "\">" . __("found here", $libsyn_text_dom) . ".</a>"; ?></em></p>
			...
			<!-- BOS Add new API -->
			  <div class="stuffbox libsyn-meta-box">
			...
			<?php if ( !isset($setup_new) ) { ?>
			  ...
			  <div class="box_left_column" id="libsyn_tabbed_settings_box">
				<div class="stuffbox libsyn-meta-box" id="libsyn_player_settings">
					...
					<div class="inside">
						...
						<table class="form-table">
								...
								<td>
									<div>
										<div style="margin-left:36px;">
											<input type="text" id="player_custom_color" class="color-picker" name="player_custom_color" value=""/><button type="button" id="player_custom_color_picker_button" class="button libsyn-dashicons-art"><?php _e('Pick Color', $libsyn_text_dom); ?></button>
										</div>
							...
							<tr valign="bottom">
								...
								<td>
									<br />
										<input type="submit" value="Save Player Settings" class="button button-primary libsyn-dashicons-check button-width-220" id="player-settings-submit" name="submit">
								</td>
							</tr>
							<tr valign="bottom">
				...
				<div class="stuffbox libsyn-meta-box" id="libsyn_additional_settings" style="display:none;">
				  <div class="inside box_left_content" style="display:none;">
					<h3>
					...
					<div class="inside">
						...
						<table class="form-table">
							...
							<?php /* DISABLE MYLIBSYN WIDGET */ $myLibsynEnabled = false; ?>
							<?php if ( !empty($selectedShow) && !empty($selectedShow->is_premium) && $myLibsynEnabled ) { ?>
							<tr valign="top">
								<th>
								<?php _e("myLibsyn Login Widget", $libsyn_text_dom); ?>
								</th>
								<td colspan="2">
									<div class="libsyn-help-tip-container" style="display:inline-grid;">
										<input style="margin-left: 2px;grid-column-start: 1;margin-top: 2px;" id="settings_enable_mylibsyn_widget" type="checkbox" value="enable_mylibsyn_widget" name="settings_enable_mylibsyn_widget" />&nbsp;<?php _e("Enable myLibsyn Widget?", $libsyn_text_dom); ?>&nbsp;
										<div style="position:relative;grid-column-start:3;margin-top:-6px;">
											<div class="libsyn-help-tip">
												<p><?php _e("Checking this box will enable the myLibsyn login widget.  Visit the
												\"Widget\"s page to add it to your site.", $libsyn_text_dom); ?></p>
											</div>
										</div>
									</div>
								</td>
			  ...
			  <div class="box_right_column">
				  <div class="stuffbox libsyn-meta-box">
					<div class="inside box_right_content_1">
						<h3>
							<label><?php _e("About", $libsyn_text_dom); ?></label>
							<label style="float:right;margin-right:1%;" id="version"></label>
						</h3>
						<div class="inside">
				  ...
				  <div class="stuffbox libsyn-meta-box">
					...
					<h3>
						<label><?php _e("Support", $libsyn_text_dom); ?></label>
					</h3>
					<div class="inside">
						<div class="box_clear"></div>
						<div style="height:24px;">
							<div style="float: left;margin-left:5%;font-weight:bold;"><?php _e("Rate 5-star", $libsyn_text_dom); ?></div>
						...
						<div class="box_clear"><hr /></div>
						<div style="height: 24px;">
							<div style="float: left;margin-left:5%;font-weight:bold;"><?php _e("Facebook Page", $libsyn_text_dom); ?></div>
							<div style="float: left;margin-left:5%;">
								<div id="fb-root"></div>
								<script>
									(function(d, s, id) {
						...
						<div class="box_clear"><hr /></div>
						<div style="height: 24px;">
							<div style="float: left;margin-left:5%;font-weight:bold;"><?php _e("Follow on Twitter", $libsyn_text_dom); ?></div>
							<div style="float: left;margin-left:5%;">
					...
					<div class="inside box_right_content_3" style="display:none">
						...
						<div class="inside">
							...
							<div style="height: 74px;" id="libsyn-download-log-containter">
								<div style="float:left;margin-left:5%;font-weight:bold;"><?php _e("Download Plugin Log", $libsyn_text_dom); ?></div>
								<div style="float:left;margin-left:5%;">
									<div class="libsyn-help-tip-container" style="display:-webkit-box;">
										<div>
											<button type="button" id="libsyn-download-log-button" class="button libsyn-dashicons-download"><?php _e("Download Log", $libsyn_text_dom); ?></button>
										</div>
										<div class="libsyn-help-tip" style="margin-left:2px;">
							...
							<div style="height: 74px;">
								<div style="float: left;margin-left:5%;font-weight:bold;"><?php _e("Reset Settings", $libsyn_text_dom); ?></div>
								<div style="float:left;margin-left:5%;">
									<div class="libsyn-help-tip-container" style="display:-webkit-box;">
										<div>
											<button type="button" id="clear-settings-button" value="Clear Settings" class="button libsyn-dashicons-trash"><?php _e("Clear Settings", $libsyn_text_dom); ?></button>
										</div>
										<div class="libsyn-help-tip" style="margin-left:2px;">
		...
		(function($){
			$(document).ready(function(){
				...
				$(".box_left_content").fadeIn('fast', function() {
					...
					var libsynPlayerTheme = '<?php if ( !empty($playerTheme) ) { echo $playerTheme; } ?>';
					if(libsynPlayerTheme == 'standard') {
					...
					} else if(libsynPlayerTheme == 'mini') {
					...
					} else if(libsynPlayerTheme == 'custom') {
					...
					} else { //default: libsynPlayerTheme is not set
					...
					<?php } ?>
					libsynPlayerColorPicker = $('#player_custom_color').iris({
					...
					libsynPlayerColorPicker.click(function(e) {
						e.preventDefault();
						if(typeof libsynPlayerColorPicker !== 'undefined') {
							libsynPlayerColorPicker.iris('show');
							libsynPlayerColorPicker.data('isOpen', 'show');
						}
					});
					...
					$('#player_custom_color_picker_button').click(function(e) {
						...
						if(typeof libsynPlayerColorPicker !== 'undefined') {
							...
							if ( typeof libsynPlayerColorPicker.data('isOpen') === 'undefined' ) {
								libsynPlayerColorPicker.iris('show');
								libsynPlayerColorPicker.data('isOpen', 'show');
							} else {
								if ( libsynPlayerColorPicker.data('isOpen') === 'show' ) {
									libsynPlayerColorPicker.iris('hide');
									libsynPlayerColorPicker.data('isOpen', 'hide');
								} else if ( libsynPlayerColorPicker.data('isOpen') === 'hide' ) {
									libsynPlayerColorPicker.iris('show');
									libsynPlayerColorPicker.data('isOpen', 'show');
								}
								libsynPlayerColorPicker.iris('toggle');
							}
					...
					//Enable myLibsyn Widget
					<?php $settingsEnableMylibsynWidget = get_option('libsyn-podcasting-settings_enable_mylibsyn_widget'); ?>
					<?php
					/* DISABLING MYLIBSYN WIDGET */
					/*
					var settingsMyLibsynWidget = '<?php if ( !empty($settingsEnableMylibsynWidget) ) { echo $settingsEnableMylibsynWidget; } ?>';
					if(settingsMyLibsynWidget == 'enable_mylibsyn_widget') {
						$('#settings_enable_mylibsyn_widget').prop('checked', true);
					}
					*/
					?>
				});
