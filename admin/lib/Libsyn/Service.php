...
class Service extends \Libsyn {
	...
	public function createLibsynApi( array $settings ) {
		...
		$data = $wpdb->get_results(
			$wpdb->prepare(
				"SELECT * FROM %i WHERE plugin_api_id = %d AND is_active = %d",
				[ $api_table_name, $lastId, 1 ]
			)
		);
	...
	public function getApi() {
		...
		if ( empty($usersmetaTable) ) { //Check table name without prefix (could be multisite)
			...
			$usersmetaTable = $wpdb->get_var( $wpdb->prepare( "SHOW TABLES LIKE %s", $prefix_modified . 'usermeta' ) );
		}

		$apiUserId = get_option('libsyn_podcasting_wp_user_id');
		...
		if ( !empty($usersmetaTable) ) {
			try {
				...
				if ( $this->current_user_id == 0 && $apiUserId ) {
					$working_user_id = $apiUserId;
				} else {
					$working_user_id = $this->current_user_id;
				}

				//try to fetch api data from usermeta
				$results = get_user_meta($working_user_id, $meta_key, true);

				if ( empty($results) || empty(maybe_unserialize($results)) ) {
					$results = get_user_meta($working_user_id, $meta_key_prefixed, true);
				}

				if ( empty($results) || empty(maybe_unserialize($results)) ) {

					if ( !$apiUserId ) {
						$userIdSearch = $wpdb->prepare(
							"SELECT meta_value FROM %i WHERE meta_key = %s OR meta_key = %s LIMIT 1",
							[ $usersmetaTable, $meta_key_prefixed, $meta_key ]
						);
					} else {
						$userIdSearch = $wpdb->prepare(
							"SELECT meta_value FROM %i WHERE meta_key = %s OR meta_key = %s AND user_id = %d LIMIT 1",
							[ $usersmetaTable, $meta_key_prefixed, $meta_key, $apiUserId ]
						);
					}

					$results = $wpdb->get_var( $userIdSearch );
				}

				if ( empty($results) || empty(maybe_unserialize($results)) ) {
					$results = $wpdb->get_var(
						$wpdb->prepare(
							"SELECT meta_value FROM %i WHERE meta_key LIKE %s",
							[ $wpdb->usermeta, '%' . $wpdb->esc_like($meta_key) ]
						)
					);
				}
		...
		} else {//try to get api usersmeta data via $wpdb
			try {
				$meta_key = $this->getApiTableName();
				$meta_key_prefixed = $wpdb->prefix . $meta_key;
				$results = $wpdb->get_var(
		...
		if ( !empty($results) ) {
			...
			if ( !empty($api_client_id) && $api_is_active ) {
				update_option( 'libsyn_podcasting_wp_user_id', $api->getUserId(), true );
				return $api;
			}
		}
		return false;
	...
	public function checkEditPostDuplicate( $itemId ){
		...
		try {
			$results = $wpdb->get_results(
				$wpdb->prepare(
					"SELECT post_id FROM %i WHERE meta_key = 'libsyn-item-id' AND meta_value = '%d'", 
					[ $wpdb->prefix . 'postmeta', $this->sanitize->itemId($itemId) ]
				)
			);
		} catch (Exception $e) {
			$results = false;
