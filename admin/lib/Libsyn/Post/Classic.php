	...
	public static function addLibsynPostMeta( $object, $box ) {
		...
		<script type="text/javascript">
			(function ($){
				$(document).ready(function() {
					...
					<?php //Show Select
						$libsynShowId	= get_post_meta( $object->ID, 'libsyn-show-id', true );
						if ( empty($libsynShowId) && $api instanceof \Libsyn\Api ) {
							$libsynShowId = $api->getShowId();
						}
					?>
					if ( !!libsyn_nmp_data.shows ) {
						var libsynShowsList = libsyn_nmp_data.shows;
					} else {
						var libsynShowsList = {};
					}

					//Check Premium
					if ( typeof libsyn_nmp_data.api.show_id !== 'undefined' && typeof libsynShowsList[libsyn_nmp_data.api.show_id].is_premium !== 'undefined' && ( libsynShowsList[libsyn_nmp_data.api.show_id].is_premium == true || libsynShowsList[libsyn_nmp_data.api.show_id].is_premium == 'true' ) ) {
						$('#libsyn-premium-state-tr').fadeIn('fast');
					} else {
						$('#libsyn-premium-state-tr').hide();
						$('#libsyn-post-episode-premium_state').val('');
					}

					//Add Show Selector
					if ( Object.keys(libsynShowsList).length >= 2 ) {
						for ( let working_show of Object.values(libsynShowsList) ) {
							if ( !!working_show.label && !!working_show.value ) {
								if ( working_show.value == <?php echo $libsynShowId; ?>) {
									var libsyn_selected_text = 'selected';
								} else {
									var libsyn_selected_text = '';
								}
								<?php
									if ( $hasLibsynReleaseDate )  {
										?>
											$("#libsyn-show-select").fadeOut("fast");
											if ( working_show.value == <?php echo $libsynShowId; ?>) {
												$("#libsyn-show-select-container").append('<strong>' + working_show.label + '</strong>');
											}
										<?php
									} else {
										?> $("#libsyn-show-select").append('<option value="' + working_show.value + '" ' + libsyn_selected_text + '>' + working_show.label + '</option>'); <?php
									}
								?>
							}
						}
						if ( typeof $("#libsyn-show-select") != 'undefined' ) {
							$("#libsyn-show-select").change(function() {
								$("#libsyn-show-select-upload-error").hide();
								if ( typeof libsyn_nmp_data != 'undefined' && typeof libsyn_nmp_data.api != 'undefined' ) {
									libsyn_nmp_data.api.show_id = $(this).val();
								}
								if ( typeof $("#libsyn-new-media-media").attr('data-show-id') != 'undefined' && !!$("#libsyn-new-media-media").attr('data-show-id') ) {
									if ( $("#libsyn-new-media-media").attr('data-show-id') != $("#libsyn-show-select").val() ) {
										$("#libsyn-show-select-upload-error").fadeIn('fast');
										libsynClearPrimaryMedia();
									}
								}
								if ( typeof libsynShowsList[libsyn_nmp_data.api.show_id].is_premium !== 'undefined' && ( libsynShowsList[libsyn_nmp_data.api.show_id].is_premium == true || libsynShowsList[libsyn_nmp_data.api.show_id].is_premium == 'true' ) ) {
									$('#libsyn-premium-state-tr').fadeIn('fast');
								} else {
									$('#libsyn-premium-state-tr').hide();
									$('#libsyn-post-episode-premium_state').val('');
		...
		<div class="libsyn-post-form libsyn-meta-box">
			<table class="form-table">
				...
				<tr valign="top" id="libsyn-show-select-tr">
					  <th>
							<label for="libsyn-show-select" style="width:20%;max-width:120px;font-weight:600;">
								<?php _e( "Select Show<span style='color:red;'>*</span>", $plugin->text_dom ); ?>
							</label>
					  </th>
					  <td>
						<div class="titlediv">
							<div class="titlewrap" id="libsyn-show-select-container">
								<select id="libsyn-show-select" name="libsyn-show-select">
									<?php //insert shows ?>
								</select>
							</div>
						</div>
					  ...
					  <td>
						<div id="libsyn-primary-media-settings">
							<div id="libsyn-new-media-settings">
								<div id="libsyn-media-upload-error" class="upload-error" style="display:none;color:red;font-weight:bold;"><?php _e('There was an error uploading media, please check settings and try again.', $plugin->text_dom); ?></div>
								<div id="libsyn-show-select-upload-error" class="upload-error" style="display:none;color:red;font-weight:bold;"><?php _e('The uploaded media did not match the selected show.  Please re-upload your episode\'s media.', $plugin->text_dom); ?></div>
								<p><?php _e( 'Select Primary Media for Episode by clicking the button below.', $plugin->text_dom ); ?></p>
								<p>
									<button class="button button-primary nth-buttons libsyn-dashicons-upload" id="libsyn-upload-media" title="<?php echo esc_attr__( 'Click here to upload media for episode', $plugin->text_dom ); ?>"><?php echo __( 'Upload Media', $plugin->text_dom ); ?></button>
									<button class="button button-primary nth-buttons libsyn-dashicons-wordpress" id="libsyn-open-media" title="<?php echo esc_attr__( 'Click Here to Open the Media Manager', $plugin->text_dom ); ?>" onclick="libsynSelectMedia(event)"><?php echo __( 'Select Wordpress Media', $plugin->text_dom ); ?></button>
									<button class="button button-primary nth-buttons libsyn-dashicons-cloud" id="libsyn-open-ftp_unreleased" title="<?php echo esc_attr__( 'Click Here to Open the Media Manager', $plugin->text_dom ); ?>" onclick="libsynSelectFtp(event)"><?php echo __( 'Select ftp/unreleased', $plugin->text_dom ); ?></button>
								</p>
								<p>
									...
									<button class="button libsyn-dashicons-close" id="libsyn-clear-media-button" title="<?php echo esc_attr__( 'Clear primary media', $plugin->text_dom ); ?>"><?php echo __( 'Clear', $plugin->text_dom ); ?></button>
								</p>
							</div>
				...
				<tr valign="top" id="libsyn-premium-state-tr">
					  <th><?php _e( "Premium State", $plugin->text_dom ); ?></th>
					  <td>
						<div class="titlediv">
							<div class="titlewrap">
								<?php $libsynEpisodePremiumState = get_post_meta( $object->ID, 'libsyn-post-episode-premium_state', true ); ?>
								<select id="libsyn-post-episode-premium_state" style="max-width:330px;" name="libsyn-post-episode-premium_state" value="<?php echo ( !empty($libsynEpisodePremiumState) ) ? $libsynEpisodePremiumState : ''; ?>">
									<option name="none" value="" <?php echo ( empty($libsynEpisodePremiumState) || $libsynEpisodePremiumState == "null" ) ? 'selected ' : ''; ?>><--<?php _e('Select an option', $plugin->text_dom); ?>--></option>
									<option name="none" value="auto" <?php echo ( !empty($libsynEpisodePremiumState) && $libsynEpisodePremiumState == "auto" ) ? 'selected ' : ''; ?>><?php _e('Auto', $plugin->text_dom); ?></option>
									<option name="none" value="premium" <?php echo ( !empty($libsynEpisodePremiumState) && $libsynEpisodePremiumState == "premium" ) ? 'selected ' : ''; ?>><?php _e('Premium', $plugin->text_dom); ?></option>
									<option name="none" value="free" <?php echo ( !empty($libsynEpisodePremiumState) && $libsynEpisodePremiumState == "free" ) ? 'selected ' : ''; ?>><?php _e('Free', $plugin->text_dom); ?></option>
								</select>
							</div>
						</div>
					  </td>
					  ...
					  <td>
						<div id="libsyn-primary-media-settings">
							<div id="libsyn-new-media-settings">
								...
								<button class="button button-primary button-width-260 libsyn-dashicons-image" id="libsyn-open-image" title="<?php echo esc_attr__( 'Click Here to Open the Image Manager', $plugin->text_dom ); ?>" onclick="libsynSelectImage(event)"><?php echo __( 'Select Episode Thumbnail', $plugin->text_dom ); ?></button></p>
								<p>
									...
									<button class="button libsyn-dashicons-close" id="libsyn-clear-image-button" title="<?php echo esc_attr__( 'Clear image url', $plugin->text_dom ); ?>"><?php echo __( 'Clear', $plugin->text_dom ); ?></button>
								</p>
							</div>
				...
				<tr valign="top">
					...
					?>
					  ...
					  <td>
						<div class="titlediv">
							<button class="button libsyn-dashicons-menu" id="libsyn-itunes-optimization-form-button" title="<?php echo esc_attr__( 'Apple Podcasts Optimization', $plugin->text_dom ); ?>" data-libsyn-wp-post-id="<?php if ( !empty($object->ID) ) { echo $object->ID; } ?>" value="false"><?php echo __( 'Apple Podcasts Optimization (Optional)', $plugin->text_dom ); ?></button>
							<div class="titlewrap">
								<br />
									...
									<fieldset>
										...
										<p style="padding-bottom:4px;">
											...
											<select id="libsyn-post-episode-itunes-episode-type" style="max-width:330px;" name="libsyn-post-episode-itunes-episode-type" value="<?php echo ( !empty($libsyn_itunes_episode_type) ) ? $libsyn_itunes_episode_type : ''; ?>">
												<option name="none" value="" <?php echo ( empty($libsyn_itunes_episode_type) || $libsyn_itunes_episode_type == "null" ) ? 'selected ' : ''; ?>><--<?php _e('Select an option', $plugin->text_dom); ?>--></option>
												<option name="none" value="full" <?php echo ( !empty($libsyn_itunes_episode_type) && $libsyn_itunes_episode_type == "full" ) ? 'selected ' : ''; ?>><?php _e('Full', $plugin->text_dom); ?></option>
												<option name="none" value="trailer" <?php echo ( !empty($libsyn_itunes_episode_type) && $libsyn_itunes_episode_type == "trailer" ) ? 'selected ' : ''; ?>><?php _e('Trailer', $plugin->text_dom); ?></option>
												<option name="none" value="bonus" <?php echo ( !empty($libsyn_itunes_episode_type) && $libsyn_itunes_episode_type == "bonus" ) ? 'selected ' : ''; ?>><?php _e('Bonus', $plugin->text_dom); ?></option>
											</select>
										</p>
										<p style="padding-bottom:4px;">
