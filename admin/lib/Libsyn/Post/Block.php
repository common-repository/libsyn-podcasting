...
class Block extends \Libsyn\Post {
	...
	public static function initBlock() {
		...
		if ( $hasApi !== false ) {
			...
			if ( !empty($user_id) ) {
				self::editorAssets();
				self::blockAssets();
				//Block Editor (Publisher)
				register_block_type('create-block/libsyn-podcasting-block', array(
						'editor_script'		=> 'libsyn_podcasting_block-editor-js',
						'editor_style'		=> 'libsyn_podcasting_block-editor-css',
						'style'				=> 'libsyn_podcasting_block-style-css',
						'title'				=> 'Libsyn Publisher Hub - Editor',
				));

				register_block_type('create-block/libsyn-podcasting-block-preview', array(
						'editor_script'		=> 'libsyn_podcasting_block-editor-js',
						// 'editor_script'		=> 'create-block/libsyn-podcasting-block',
						// 'editor_style'		=> 'libsyn_podcasting_block-editor-css',
						// 'style'				=> 'libsyn_podcasting_block-style-css',
						'title'				=> 'Libsyn Publisher Hub - Preview',
						// 'icon'				=> 'format-video',
						'parent'			=> 'create-block/libsyn-podcasting-block',
						...
						'attributes'		=> array(
							...
							'playerShortcode' => array(
								'type'		=> 'string',
								'source'	=> 'html',
								'default'	=> ''
							),
							'playerEmbedurl' => array(
								'type'		=> 'string',
								'source'	=> 'html',
								'default' => ''
							)
				...
				// var_dump(get_dynamic_block_names());exit;
			}
		} else {
	...
	public static function blockAssets() {
		// Styles.
		wp_register_style(
			'libsyn_podcasting_block-style-css', // Handle.
			plugins_url( LIBSYN_DIR . '/lib/libsyn-podcasting-block/build/style-index.css' ), // Block style CSS.
	...
	public static function editorAssets() {
		// Scripts.
		wp_register_script(
			'libsyn_podcasting_block-editor-js', // Handle.
			plugins_url( LIBSYN_DIR . '/lib/libsyn-podcasting-block/build/index.js' ), // We register the block here. Built with Webpack.
			array( 'wp-blocks', 'wp-i18n', 'wp-editor', 'wp-components', 'wp-element' ), // Dependencies, defined above.
			null, //Plugin Version (Not required)
			true //Load in Footer (Fixes bug caused by not saving some post meta)
		);

		// Styles.
		wp_register_style(
			'libsyn_podcasting_block-editor-css', // Handle.
			plugins_url( LIBSYN_DIR . '/lib/libsyn-podcasting-block/build/index.css' ), // Block editor CSS.
	...
	public static function shortcodeCallback( $attributes ) {
		...
		$atts['src'] = ( !empty($attributes['playerEmbedurl']) ) ? $attributes['playerEmbedurl'] : get_post_meta($current_post_id, 'libsyn-episode-embedurl', true);
		...
		$atts['libsyn-item-id'] = ( !empty($attributes['playerItemId']) ) ? $attributes['playerItemId'] : get_post_meta($current_post_id, 'libsyn-item-id', true);
	...
	public static function savePreviouslyPublishedMeta( $libsynPost, $current_post_id ) {
		...
		if ( !empty($libsynPost->item_title) ) {
			if ( function_exists('wp_update_post') ) {
				...
				$post_content .= '<!-- wp:create-block/libsyn-podcasting-block --><div class="wp-block-create-block-libsyn-podcasting-block"><div class="libsyn-shortcode"></div></div><!-- /wp:create-block/libsyn-podcasting-block -->';
