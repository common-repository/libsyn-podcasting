...
class Page extends \Libsyn\Service {
	...
	public static function addMeta() {
		...
		if ( $plugin instanceof \Libsyn\Service ) {
							...
							if ( !empty($postMeta) && is_array($postMeta) ) {
								...
								if ( !empty($libsynMeta['libsyn-show-show_title']) && !empty($libsynMeta['libsyn-show-feed_url']) ) {
									...
									if ( $addPodcastMetadata === 'add_podcast_metadata' ) {//Add Metadata Active
										?><link rel="alternate" type="application/rss+xml" title="<?php echo $libsynMeta['libsyn-show-show_title']; ?>" href="<?php echo $libsynMeta['libsyn-show-feed_url']; ?>" /><?php
									}
								}
							}
						}
			...
			if ( $showDefault && ( $api = $plugin->getApi() && !empty($api) && $api instanceof \Libsyn\Api ) ) {//Api Found
				//Default meta
				if ( $addPodcastMetadata === 'add_podcast_metadata' ) {//Add Metadata Active
					?><link rel="alternate" type="application/rss+xml" title="<?php echo $api->getShowTitle(); ?>" href="<?php echo $api->getFeedUrl(); ?>" /><?php
				}
			}
