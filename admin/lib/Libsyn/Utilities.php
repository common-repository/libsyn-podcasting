...
class Utilities extends \Libsyn {
	...
	public static function getUserOptionKeys() {
		return array(
			...
			'libsyn-podcasting-feed_import_libsyn_feed',
			'libsyn-podcasting-feed_import_posts',
			'libsyn-podcasting-imported_post_ids',
			'libsyn-podcasting-import_posts_data',
			'libsyn-podcasting-feed_import_locked_status',
			'libsyn-podcasting-feed_import_owner_email',
	...
	public static function getSiteOptionKeys() {
		return array(
			...
			'libsyn-podcasting-settings_add_podcast_metadata',
			'libsyn-podcasting-settings_use_classic_editor',
			'libsyn-podcasting-settings_enable_mylibsyn_widget',
		);
	}
	...
	public static function saveOauthSettings() {
		...
		if ( intval($urlParams['libsyn_oauth_settings']) === 1 ) {
			...
			if ( !empty($clientId) ) $ret = array('client_id' => $clientId, 'client_secret' => $clientSecret);
				else $error = true;

			//set output
			header('Content-Type: application/json');
			if ( !$error ) echo json_encode($ret);
				else echo json_encode(array());
			exit;
		}
	}
	...
	public static function uninstallSettings() {
			...
			foreach ( $option_names as $option ) {
				...
				if ( function_exists('delete_option') ) {
					if ( !delete_option( $option ) ) {
						if ( function_exists('update_option') ) {//user may not have delete privs on database
							update_option( $option, array() ); //fill with empty array
						}
					}
				}

				//Delete Settings for User Meta
				if ( $plugin->hasLogger ) $plugin->logger->info("Utilities::deactivateSettings:\tClearing User Option:\t" . $option);
				if ( function_exists('delete_user_option') ) {
					if ( !delete_user_option($current_user_id, $option, false) && !delete_user_option($current_user_id, $option, true) ) {
						if ( function_exists('update_user_option') ) {//user may not have delete privs on database
							$userOption = get_user_option($option, $current_user_id);
							if ( !empty($userOption) ) {
				...
				if ( function_exists('delete_site_option') && function_exists('is_multisite') && is_multisite() ) {
					if ( !delete_site_option( $option ) ) {//user may not have delete privs on database
						update_site_option($option, array()); //fill with empty array
						if ( $plugin->hasLogger ) $plugin->logger->info("Utilities::uninstallSettings:\tClearing Multisite Option:\t" . $option);
					}
				}
		...
		if ( !empty($api_table_name) ) {
			try {
				$wpdb->query( $wpdb->prepare( "DROP TABLE IF EXISTS %i", $api_table_name ) ); //old without prefix
				$wpdb->query( $wpdb->prepare( "DROP TABLE IF EXISTS %i", $wpdb->prefix . $api_table_name ) );
			} catch( Exception $e ) {
				if ( $plugin->hasLogger ) $plugin->logger->error("Utilities::uninstallSettings:\t" . $e);
				return false;
			}
	...
	public static function deactivateSettings() {
		try {
			...
			if ( function_exists('delete_user_option') ) {
				if ( $plugin->hasLogger ) $plugin->logger->info("Utilities::deactivateSettings:\tPurging API Settings");
				if ( !delete_user_option($current_user_id, $api_table_name, false) && !delete_user_option($current_user_id, $api_table_name, true) ) {
					if ( $plugin->hasLogger ) $plugin->logger->info("Utilities::deactivateSettings:\tAPI Settings Purge Failed!");
					if ( function_exists('update_user_option') ) {
						$options = get_user_option($api_table_name, $current_user_id);
						if ( !empty($options) ) {
							if ( $plugin->hasLogger ) $plugin->logger->info("Utilities::deactivateSettings:\tClearing API Settings");
							update_user_option($current_user_id, $api_table_name, json_encode($dataSettings));
						}
					}
				}
	...
	public static function logPluginData() {
		...
		if ( intval($urlParams['libsyn_debuginfo']) === 1 ) {
			if ( ( is_user_logged_in() && current_user_can( 'manage_options' ) ) === false ) {//check to make sure user is logged in and has access
				//set output
				header('Content-Type: application/json');
				echo "";
				exit;
			...
			if ( function_exists('get_bloginfo') ) {
				try {
					global $wp_version;
					$line = '';
					if ( $plugin->hasLogger ) $plugin->logger->info("Plugins:\tGeneral Wordpress Information.");
					...
					if ( $plugin->hasLogger ) $plugin->logger->info($line);
					if ( isset($line) ) unset($line);
				} catch ( Exception $e ) {
					if ( $plugin->hasLogger ) $plugin->logger->error("Utilities::logPluginData:\tUnable to get blog info.\t" . $e->getMessage());
				}
			}
			...
			if ( function_exists('get_plugins') ) {
				try {
					...
					if ( is_array($all_plugins) ) {
						if ( $plugin->hasLogger ) $plugin->logger->info("Plugins:\tGenerating list of installed plugins.");
						foreach($all_plugins as $pluginName => $pluginInfo) {
							if ( !empty($pluginName) ) {
								...
								if ( !empty(array_intersect($active_plugins, array($pluginName))) ) {
									$line .= "\n\t\tActive:\tTRUE";
								} else {
									$line .= "\n\t\tActive:\tFALSE";
								}
								...
								if ( !empty($pluginInfo['RequiresWP']) ) {
									$line .= "\n\t\tRequiresWP:\t" . $pluginInfo['RequiresWP'];
								}
								if ( !empty($pluginInfo['RequiresPHP']) ) {
									$line .= "\n\t\tRequiresPHP:\t" . $pluginInfo['RequiresPHP'];
								}
								if ( $plugin->hasLogger ) $plugin->logger->info($line);
							}
							if ( isset($line) ) unset($line);
					...
					if ( $plugin->hasLogger ) $plugin->logger->error("Utilities::logPluginData:\tUnable to get plugins data.\t" . $e->getMessage());
				}
			}
			...
			if ( function_exists('wp_get_theme') ) {
				try {
					...
					$line = '';
					if ( $plugin->hasLogger ) $plugin->logger->info("Plugins:\tActive Theme Information.");
					...
					if ( $plugin->hasLogger ) $plugin->logger->error("Utilities::logPluginData:\tUnable to get theme data.\t" . $e->getMessage());
				}
			}
		...
		try {
			...
			$check = $wpdb->get_results(
			    $wpdb->prepare(
			        ...
			        FROM                %i AS pm
			        JOIN                %i AS p ON pm.post_id = p.ID
			        WHERE               meta_key = %s
			        AND                 post_status = 'publish'
					AND					meta_value = %s
			        ORDER BY            meta_value ASC
			        ",
			        [ $wpdb->postmeta, $wpdb->posts, $postMeta_key, $postMeta_value ]
			    )
			);
			if ( $check ) {
				return true;
	...
	public function get_id_from_guid( $guid ){
		global $wpdb;
		return $wpdb->get_var( $wpdb->prepare( "SELECT ID FROM %i WHERE guid = %s", [ $wpdb->posts, $guid ] ) );
	}
