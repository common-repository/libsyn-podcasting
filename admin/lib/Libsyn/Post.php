...
class Post extends \Libsyn {
	...
	public static function addLocalizedAssets() {
		...
		if ($api instanceof \Libsyn\Api ) {
			...
			//myLibsyn
			// $enableMyLibsyn = get_option('libsyn-podcasting-settings_enable_mylibsyn_widget');
			// $localization['enable_mylibsyn_widget'] = ( !empty($enableMyLibsyn) ) ? true : false;
...
/* 			$attributes = array( 'attributes' =>
				array (
					'src' => 'https://www.wordpresssite.com/blah',
					'theme' => 'custom'
				)
			);
			...
			switch ( $postEditorType ) {
				case 'block':
					...
					$localization['error']['destinations_data'] = false;
					...
					if ( !empty( $destinations->destinations ) ) {
						...
						if ( is_array($destinations->destinations) || is_object($destinations->destinations) ) {
						...
						} else {//handle error if any
							$localization['destinations'] = array();
							$localization['error']['destinations_data'] = true;
							if ( $plugin->hasLogger ) $plugin->logger->error( "Post:\taddLocalizedAssets:\t" . $destinations->destinations );
						}
					} else {//handle error if any
						$localization['destinations'] = array();
						$localization['error']['destinations_data'] = true;
						if ( $plugin->hasLogger ) $plugin->logger->error( "Post:\taddLocalizedAssets:\tUnable to fetch destinations data." );
					}
	 ...
	 *
	 * @since 1.0.1.1
	 * @param  int $id  WP post id ($object->ID)
	 * @param string $prefix
	...
	public static function handlePost( $post_id, $post ) {
		...
		if ( $postEditorType == 'classic' ) {
			...
			// $new_meta_values['libsyn-post-episode-itunes-episode-summary'] = (isset($_POST['libsyn-post-episode-itunes-episode-summary'])) ? $_POST['libsyn-post-episode-itunes-episode-summary'] : ''; @deprecated v1.3.1
			...
			$new_meta_values['libsyn-post-episode-premium_state'] = (isset($_POST['libsyn-post-episode-premium_state'])) ? $_POST['libsyn-post-episode-premium_state'] : '';
	...
	public static function postEpisode( $post, $isSchedule=false, $isDraft=false ) {
		...
		// $item['itunes_episode_summary'] = get_post_meta($post->ID, 'libsyn-post-episode-itunes-episode-summary', true); @deprecated v1.3.1
		...
		$item['premium_state'] = get_post_meta($post->ID, 'libsyn-post-episode-premium_state', true);
		...
		if ( empty($item['player_use_thumbnail']) ) {
			$item['player_use_thumbnail'] = get_user_option('libsyn-podcasting-player_use_thumbnail');
			if ( empty($item['player_use_thumbnail']) ) {
	...
	public static function loadFormData( $params = array() ) {
		...
		if ( function_exists('current_user_can') && current_user_can( 'upload_files' ) ) {
			$userUploadFilesPermission = true;
		} else {
			$userUploadFilesPermission = false;
			if ( $plugin->hasLogger ) $plugin->logger->error( "Post:\tloadFormData:\tCurrent user does not have permission to upload files." );
		}

		if ( function_exists('current_user_can') && current_user_can( 'edit_posts' ) ) {
			$userEditPostsPermission = true;
		} else {
			$userEditPostsPermission = false;
			if ( $plugin->hasLogger ) $plugin->logger->error( "Post:\tloadFormData:\tCurrent user does not have permission to edit posts." );
		}

		if ( intval($urlParams['load_libsyn_media']) == 1 && $userUploadFilesPermission && $userEditPostsPermission ) {

			/* Remove existing FTP/Unreleased Media */
			$check_ftpUnreleased = self::removeFTPUnreleased( array('action' => 'remove_ftp_unreleased', 'remove_ftp_unreleased' => '1') );
			if ( !$check_ftpUnreleased ) {
				$libsyn_error = true;
				if ( $plugin->hasLogger ) $plugin->logger->error( "Post:\tloadFormData:\tSomething went wrong when loading categories." );
			}
			...
			$api = $plugin->retrieveApiById($current_user_id);
			if ( $api instanceof \Libsyn\Api ) {
				if ( !empty( $urlParams['show_id'] ) ) {
					$sanitize = new \Libsyn\Service\Sanitize();
					$api->setShowId( $sanitize->showId($urlParams['show_id']) );
				}
				...
				if ( $refreshApi ) { //successfully refreshed
					...
					try {
							...
							foreach( $ftp_unreleased as $media ) {
								...
								if ( ( strpos($media->mime_type, 'audio') !== false ) || ( strpos($media->mime_type, 'video') !== false ) ) {
									...
									$data = array(
										...
										'post_mime_type'		=>	$mime_type_prefix . '/ftp-unreleased',
										'comment_count'			=>	0,
									);
						...
						$libsyn_error = true;
						if ( $plugin->hasLogger ) $plugin->logger->error( "Post:\tloadFormData:\tUnable to load ftp/unreleased media.\tError:\t" . $e->getMessage() );
					}
     ...
     * @return void
     */
	public static function removeFTPUnreleased( $params = array() ) {
		global $wpdb;
		$libsyn_error = true;
		$checkUrl  = self::getCurrentPageUrl();
		if ( empty($params) ) {
		...
		if ( intval($urlParams['remove_ftp_unreleased']) === 1 ) {
			try {
				...
				$wpdb->query(
					$wpdb->prepare(
						"DELETE FROM %i WHERE meta_value LIKE %s",
						[ $wpdb->prefix . 'postmeta', '%libsyn/ftp-unreleased%' ]
					)
				);
			} catch ( Exception $e ) {
				$plugin = new Service();
				if ( $plugin->hasLogger ) $plugin->logger->error( "Post:\tremoveFTPUnreleased:\t" . $e->getMessage() );
			}
			$libsyn_error = false;
